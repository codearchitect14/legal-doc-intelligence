...
Run at: 2026-09-15T17:37:19.557124+00:00
Base URL: http://localhost:8000

## 1. Firm registration and login
- Registered firm and admin user `demo-walkthrough-986c8c58@example.com`, status 201
- Logged in, access token acquired

## 2. Case creation
- Created case `d33523d8-5b15-418f-8ee7-f9c2875c052d`

## 3. Document upload (real files from data/sample_case_folder/)
- Uploaded 4 real documents in 0.27s:
...
- Processing settled after 66.95s
  - `80a4d76e...` -> category=`uncertain`, ocr_status=`not_required`
  - `fef4a670...` -> category=`filing`, ocr_status=`not_required`
  - `294b5afc...` -> category=`contract`, ocr_status=`not_required`
  - `a82e358e...` -> category=`uncertain`, ocr_status=`not_required`

## 5. Analysis (chronology, totals, inconsistencies)
- Analysis completed in 0.27s
...
## 6. Draft generation
- `chronology_summary`: status=`complete` in 0.05s
  - Content length: 8305 characters
- `demand_letter`: status=`complete` in 2.58s
  - Content length: 4391 characters
...
## Summary
- Total wall-clock time, upload through both drafts: 70.57s
